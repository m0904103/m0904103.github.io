      ...
      <header className="px-4 md:px-8 py-4 border-b border-white/5 bg-[#0B0E11]/80 backdrop-blur-xl sticky top-0 z-50 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex space-x-2">
            <a href="https://m0904103.github.io/youtube-stock-tracker/" target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1 px-2 md:px-3 py-1.5 bg-[#161A1E] hover:bg-gray-800 border border-white/10 rounded-lg text-xs font-bold text-gray-300 transition-colors">
              <ExternalLink size={14} className="text-blue-400" />
              <span className="hidden sm:inline">YT 追蹤器</span>
              <span className="sm:hidden">YT</span>
            </a>
            <a href="https://m0904103.github.io/trump-advisory/" target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1 px-2 md:px-3 py-1.5 bg-[#161A1E] hover:bg-gray-800 border border-white/10 rounded-lg text-xs font-bold text-gray-300 transition-colors">
              <ExternalLink size={14} className="text-red-400" />
              <span className="hidden sm:inline">川普顧問</span>
              <span className="sm:hidden">川普</span>
            </a>
          </div>
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: '30px', background: '#111827', color: '#f87171', fontFamily: 'sans-serif', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ maxWidth: '600px', width: '100%', background: '#1f2937', padding: '30px', borderRadius: '16px', border: '1px solid #374151', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.5)' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#f87171', marginBottom: '10px' }}>🚨 檢測到舊版瀏覽器快取殘留 (Cache Detected)</h2>
            <p style={{ fontSize: '14px', color: '#9ca3af', marginBottom: '15px' }}>
              系統已成功修復此問題！此畫面是由於您的瀏覽器快取了舊版的 JS 檔案所致。請點擊下方按鈕以強制載入最新版本：
            </p>
            <pre style={{ background: '#111827', padding: '12px', borderRadius: '8px', color: '#fca5a5', fontSize: '12px', overflow: 'auto', marginBottom: '20px' }}>
              {this.state.error && this.state.error.toString()}
            </pre>
            <button 
              onClick={this.handleHardReload}
              style={{ width: '100%', padding: '14px', background: '#3b82f6', color: '#ffffff', fontWeight: 'bold', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '15px' }}
            >
              🔄 一鍵清除舊快取並載入最新版本 (Purge Cache & Load Live)
            </button>
          </div>
        </div>
      );
