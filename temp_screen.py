...
filtered = [s for s in tw_stocks if s.get('change', 0) is not None and s.get('close', 0) > s.get('ma60', 99999)]
filtered.sort(key=lambda x: x.get('change', 0), reverse=True)

print("--- 🚀 強勢突破 (Top 10 TW Gainers Above MA60) ---")
for s in filtered[:10]:
    gap20 = (s['close'] - s.get('ma20', s['close'])) / s.get('ma20', s['close']) * 100 if s.get('ma20') else 0
    print(f"{s['symbol']} ({s.get('name', '')}) : {s.get('change', 0)}% - {s.get('sector', '')} | 乖離月線: {gap20:.1f}%")

# Pullbacks close to MA20 (Safe Entries)
pullbacks = [s for s in tw_stocks if s.get('close', 0) > s.get('ma60', 99999) and s.get('ma20')]
for s in pullbacks:
